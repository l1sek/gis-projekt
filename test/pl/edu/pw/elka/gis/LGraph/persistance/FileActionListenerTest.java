...
public class FileActionListenerTest {
    ...
    public static Iterable<? extends Object> data() throws URISyntaxException {
        File[] testFiles = new File(FileActionListenerTest.class.getClassLoader().getResource("resources").toURI()).listFiles();
        List<String> testCases = new ArrayList<>();
        for(File file : testFiles){
            if(file.getName().endsWith("_input.graph")){
                testCases.add(file.getName().replace("_input.graph", ""));
            }
        }
        return testCases;
    }
    ...
    @Test
    public void successfulLoadSaveGraphTest() throws IOException, InvalidFileFormatException, URISyntaxException {
        //given
        InputStream inputStream = FileActionListenerTest.class.getClassLoader().getResourceAsStream(testCase+"_input.graph");
        String expectedGraph = new String(Files.readAllBytes(Paths.get(FileActionListenerTest.class.getClassLoader().getResource(testCase+"_output.graph").toURI())));
        String expectedLineGraph = new String(Files.readAllBytes(Paths.get(FileActionListenerTest.class.getClassLoader().getResource(testCase+"_output.lgraph").toURI())));
